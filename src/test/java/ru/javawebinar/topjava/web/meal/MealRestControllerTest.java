...
public class MealRestControllerTest extends AbstractControllerTest {
    ...
    void createNew() throws Exception {
        ...
        Meal result = readFromJson(perform(MockMvcRequestBuilders.post(REST_URL)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(JsonUtil.writeValue(meal)))
                        .andExpect(status().isCreated()),
                Meal.class);
        meal.setId(result.id());
        MEAL_MATCHER.assertMatch(result, meal);
        MEAL_MATCHER.assertMatch(mealService.get(result.id(), USER_ID), meal);
    }
